...
const search_es = async (ctx) => {
    ...
    if (ret_num == 0) {
        ...
        let ctxbody = { is_zh: is_zh, tokens: broken_tokens, results: tmp }
        ctx.body = ctxbody
        ctx.status = 200
    }
};
...
const search = async (ctx) => {
    ...
    if (category == 0) {
        res0 = await BookModel.find({
            $or: [{ "bookname": { $regex: searchQuery, "$options": "i" } },
        ...
        res0 = await BookModel.find({
            $or: [{ $and: [{ "bookname": { $regex: searchQuery, "$options": "i" } }, { "category": catg[category] }] },
            { $and: [{ "author": { $regex: searchQuery, "$options": "i" } }, { "category": catg[category] }] }]
        }).skip(skip).limit(limit).exec();
